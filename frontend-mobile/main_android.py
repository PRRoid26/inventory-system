...
class AutocompleteField(BoxLayout):
    """
    MDTextField + live dropdown suggestions from a history list.
    As the user types, matching items appear in an MDDropdownMenu.
    Selecting a suggestion fills the field and closes the menu.
    """

    def __init__(self, hint_text="", history_fn=None, **kwargs):
        """
        history_fn: callable that returns List[str] of candidates
        """
        super().__init__(orientation="vertical",
                         size_hint_y=None, spacing=0, **kwargs)
        self.bind(minimum_height=self.setter("height"))

        self._history_fn = history_fn or (lambda: [])
        self._menu       = None

        self.field = MDTextField(hint_text=hint_text,
    ...
    def _on_text(self, field, value):
        ...
        matches = [h for h in self._history_fn()
                   if query in h.lower()][:8]
        if not matches:
            return
        ...
        items = [
            {
                ...
                "on_release": lambda x=m: self._pick(x),
            }
            for m in matches
        ]
...
class LoginScreen(Screen):
    ...
    def _build(self):
        ...
        self.reg_btn = MDFlatButton(
            text="Don't have an account? Register",
            size_hint=(1, None), height=dp(36),
            theme_text_color="Custom",
            text_color=(0.18, 0.55, 0.95, 1))
        self.reg_btn.bind(on_release=self._open_register)
        root.add_widget(self.reg_btn)
    ...
    def _open_register(self, *_):
        r_name  = MDTextField(hint_text="Full Name",
                              size_hint_y=None, height=dp(52))
        r_user  = MDTextField(hint_text="Username",
                              size_hint_y=None, height=dp(52))
        r_email = MDTextField(hint_text="Email",
                              size_hint_y=None, height=dp(52))
        r_pass  = MDTextField(hint_text="Password", password=True,
                              size_hint_y=None, height=dp(52))
        r_pass2 = MDTextField(hint_text="Confirm Password", password=True,
                              size_hint_y=None, height=dp(52))
        r_err   = MDLabel(text="", font_style="Caption",
        ...
        for w in [r_name, r_user, r_email, r_pass, r_pass2, r_err]:
            content.add_widget(w)

        def _do_register(*_):
            r_err.text = ""
            if not r_user.text.strip():
                r_err.text = "Username is required."
                return
            if not r_email.text.strip():
                r_err.text = "Email is required."
                return
            if len(r_pass.text) < 6:
                r_err.text = "Password must be at least 6 characters."
                return
            if r_pass.text != r_pass2.text:
                r_err.text = "Passwords do not match."
                return

            save_btn.disabled = True
            save_btn.text = "Registering..."

            def _api():
                return self.api_client.register(
                    username=r_user.text.strip(),
                    email=r_email.text.strip(),
                    password=r_pass.text,
                    full_name=r_name.text.strip(),
                )

            def _done(ok):
                save_btn.disabled = False
                save_btn.text = "Register"
                if ok:
                    dlg.dismiss()
                    show_snack("Account created! Logging you in...")
                    self.on_login()
                else:
                    r_err.text = "Registration failed. Username or email may already exist."

            run_async(_api, _done)

        save_btn = MDRaisedButton(
            text="Register", md_bg_color=(0.18, 0.55, 0.95, 1))
        save_btn.bind(on_release=_do_register)
...
class ITAssetApp(MDApp):
    ...
    def open_change_status(self):
        ...
        self._cs_loc      = AutocompleteField(hint_text="Location",
                                              history_fn=_location_history.items)
        ...
        # Recent locations hint (type to get dropdown suggestions)
        recent = _location_history.items()[:3]
        loc_hint = MDLabel(
            text=("Recent: " + "  |  ".join(recent)) if recent else "Type to search saved locations",
        ...
        for w in [self._cs_asset, self._cs_hint, self._cs_status_btn,
                  self._cs_loc, loc_hint, self._cs_dept,
